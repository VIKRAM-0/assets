...
- `lib/engine.js`: `E` + `BASE_TILE, texLoader, escapeHtml, showToast, setSliderVal, tryLoadTex, makeGreyscaleTex, POLY_NORM_KEYS, POLY_ROUGH_KEYS, POLY_DIFF_KEYS, pickPolyUrl, saveMaterialSnapshot, markDirty, roomElements, roomVisible, roomFurnitureModels, modelMaterialSnapshots, _gltfSceneCache, polyCache, texCache, enhanceCache, CURTAIN_FABRICS, CURTAIN_COLORS, CURTAIN_COLOR_GROUPS, raycaster, mouse` (export every remaining top-level binding read by another module — verify with the derivation grep in Step 2)
